...
const config = {
  paths: [
    ...
    {
      schemas: '/prototyping/general/schemas',
  ...
  try {
    ...
    for( const filename of files ) {
      ...
      if( stat.isFile() ) {
        ...
        let converter = new GenerateMarkdown(parsedSchema, title, paths);
...
class GenerateMarkdown {
  ...
  convertSchema() {
    // add base level properties
    let sections = [{ key: this.name, values: this.schema }];
    let links = [];
    ...
    return this.markdownOutput({ sections, links });
  }

  schemaToImageUrl(url) {
    // brittle
    url = url.replace('/schemas/', '/diagrams/');
